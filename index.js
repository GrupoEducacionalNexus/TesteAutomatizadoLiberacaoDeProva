...
(async () => {
    ...
    if (opcaoCursos) {
        ...
        if (opcaoGestaoDeCursos) {
            ...
            page.waitForSelector('body > div.page-container.row-fluid > div.page-content > div > div:nth-child(11) > table > tbody > tr:nth-child(6) > td:nth-child(11) > div > img');
            await new Promise((resolve) => setTimeout(resolve, 3000)); // Aguarda 3 segundos antes de acessar o próximo elemento
            const btnAtualizar = await page.$('body > div.page-container.row-fluid > div.page-content > div > div:nth-child(11) > table > tbody > tr:nth-child(6) > td:nth-child(11) > div > img'); //pegar referencia do curso
            
            if (btnAtualizar) {
                await btnAtualizar.click();
               
                await page.goto('https://cbi.sistemasiga.net/curso/atualizar/34', { waitUntil: ['load', 'networkidle0'] });
                console.log('Página atual:', page.url());
                ...
                if (btnEad) {
                    ...
                    if (btnAulas) {
                        ...
                        const btn7AnoFundamental = await page.$('#tab_3_6_2 > div > ul > li:nth-child(3) > a');
                        if (btn7AnoFundamental) {
                            btn7AnoFundamental.click();
                            ...
                            const arrayCheckbox = await page.$$(`.check23`);
                            ...
                            const result = await Promise.all(arrayInputLiberacaoAulaDataFixa.map(async (t) => {
                                return await t.evaluate(x => x.value = `14/07/2023`);
                            }))
                            ...
                            const result2 = await Promise.all(arrayInputBloqueioAulaDataFixa.map(async (t) => {
                                return await t.evaluate(x => x.value = `15/07/2023`);
                            }))
